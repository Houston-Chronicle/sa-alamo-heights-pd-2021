...
%>%
  mutate(word_cnt_2 = case_when(
    offenses_2 != "" ~ lengths(gregexpr("\\W+", offenses_2)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_3 = case_when(
    offenses_3 != "" ~ lengths(gregexpr("\\W+", offenses_3)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_4 = case_when(
    offenses_4 != "" ~ lengths(gregexpr("\\W+", offenses_4)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_5 = case_when(
    offenses_5 != "" ~ lengths(gregexpr("\\W+", offenses_5)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_6 = case_when(
    offenses_6 != "" ~ lengths(gregexpr("\\W+", offenses_6)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_7 = case_when(
    offenses_7 != "" ~ lengths(gregexpr("\\W+", offenses_7)) + 1,
    TRUE ~ 0
  )) %>%
  mutate(word_cnt_8 = case_when(
    offenses_8 != "" ~ lengths(gregexpr("\\W+", offenses_8)) + 1,
    TRUE ~ 0
  )) 
...
incidents_fixing <- incidents_longer %>%
  filter(value_clean == "POSSESSION OF OPEN CONTAINER IN")
...
incident_off_fix <- incidents_longer %>%
  filter(grepl("REQUIREMENTS ON STRIKING UNATTENDED VEH", offenses_clean))
