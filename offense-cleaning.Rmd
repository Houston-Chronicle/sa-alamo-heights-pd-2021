...
I found a small error with thefts — it was with two data points so I went ahead and cleaned that up by editing the original data point and creating two new ones

```{r}
theft_1 <- data.frame(4330, "12-01432(0)", as.Date("2012-06-27 21:59:00"), as.Date("2012-06-01"), "NUGENT J A", "4821 BROADWAY ALAMO HEIGHTS TX 78209", "THEFT < $50", "CONSUMPTION OF ALCOHOL BY A MINOR THEFT < $50", "THEFT < $50")

theft_2 <- data.frame (8205, "13-02669(0)", as.Date("2013-12-23 04:18:00"), as.Date("2013-12-01"), "CARNEY M I", "5800 BLK BROADWAY ALAMO HEIGHTS TX 78209", "THEFT > $1500 < $20k", "POSSESSION DANGEROUS DRUGS POSSESSION OR DELIVERY OF DRUG PARAPHERNALIA THEFT >= $50 < $500 THEFT < $50 THEFT > $1500 < $20k POSS CS PG 1 < 1G", "THEFT < $50")

names(theft_1) <- c("rowid", "incident_num", "date", "incident_month", "officer", "address_clean", "desc_clean", "offenses_clean", "value_clean")

names(theft_2) <- c("rowid", "incident_num", "date", "incident_month", "officer", "address_clean", "desc_clean", "offenses_clean", "value_clean")

clean_offenses <- rbind(incidents_longer, theft_1, theft_2) 

clean_offenses <- clean_offenses %>%
  mutate(value_clean = case_when(
    value_clean == "CONSUMPTION OF ALCOHOL BY A MINOR THEFT < $50" ~ "CONSUMPTION OF ALCOHOL BY A MINOR",
    value_clean == "THEFT >= $50 < $500 THEFT < $50" ~ "THEFT >= $50 < $500",
    TRUE ~ value_clean
  )) %>%
  mutate(value_clean = case_when(
    value_clean == "COV - OPERATE ALARM SYSTEM W/ O PERMIT" | value_clean == "COV - OPERATINGING ALARM SYSTEM W/ O PERMIT" ~ "COV - OPERATING ALARM SYSTEM W/ O PERMIT", 
    TRUE ~ value_clean
  ))
```
...
```{r}
write_csv(full_incident_code_desc, "Data/Exports/full_incident_code_desc_2021.csv")
```



```{r}
offenses_2020 <- types_of_offenses_clean %>%
...
offenses_2020 <- offenses_2020[1:10,]
...
```{r}
offenses_2020_in_21 <- types_of_offenses_clean %>%
  filter(year == as.Date("2021-01-01")) %>%
  group_by(value_clean) %>%
  tally() %>%
  rename(count_2021 = n) %>%
  inner_join(offenses_2020)

offenses_2020_in_21
```

```{r}
addresses_2021 <- types_of_offenses_clean %>%
  filter(year == as.Date("2021-01-01")) %>%
...
```{r}
offenses_2021 <- types_of_offenses_clean %>%
  filter(year == as.Date("2021-01-01")) %>%
  group_by(value_clean, value_clean_OG) %>%
  tally() %>%
  arrange(desc(n))
...
```{r}
write.csv(offenses_2020_in_21, "Data/offenses_2020_in_21.csv")
write.csv(offenses_2021, "Data/offenses_2021.csv")
```
...
incidents_fixing <- incidents_longer %>%
  filter(value_clean == "911 INC/ SILENT/ABUSE") 
