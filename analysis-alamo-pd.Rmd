...
```{r}
codes_classified <- read_excel("Data/Exports/FOR REVIEW/march_14_codes_official.xlsx", sheet = 2)
...
```
```{r}
values_doubled_check <- codes_classified %>%
  select(value_clean) %>%
  distinct()
...
```{r}
full_incidents_pd_approved <- clean_offenses %>%
  inner_join(clean_codes_classified, by = "value_clean")

full_incidents_pd_approved
```
...
```{r}
year_count <- full_incidents_pd_approved %>%
...
```{r}
priority_year_grouping <- full_incidents_pd_approved %>%
...
```{r}
property_crime_year_grouping <- full_incidents_pd_approved %>%
...
```{r}
chief_approved_incident <- full_incidents_pd_approved %>%
  ungroup() %>%
...
```{r}
chief_approved_incident_property <- full_incidents_pd_approved %>%
  ungroup() %>%
