...
clean_nibrs <- nibrs %>%
  ...
  select(clean_offenses, type, class, nibr, ucr, priority) 
...
clean_alamo_codes_priority <- alamo_codes_priority %>%
  select(offense_description, type, class, nibr, ucr, priority) %>%
...
alamo_codes <- clean_alamo_codes_priority %>%
  full_join(clean_nibrs, by = c("clean_offenses", "type", "class", "nibr", "ucr", "priority")) %>%
