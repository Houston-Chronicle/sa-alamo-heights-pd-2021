...
clean_2011 <- raw_2011 %>%
  ...
  mutate(offenses_9 = case_when(
    is.na(Date) & is.na(`Incident Address`) & is.na(incident_num) & !is.na(lag(offenses_8)) ~ `Offenses`,
  )) %>%
...
#now mutate the original columns to get rid of the "extra" information and bring up the information to the proper row
  ...
  mutate(offenses_9 = case_when(
    !is.na(Offenses) ~ lead(lead(lead(lead(lead(lead(lead(lead(offenses_9)))))))),
    TRUE ~ offenses_9
  )) %>%
  ...
  mutate(offenses_clean = paste(offenses_clean_p1, offenses_3, offenses_4, offenses_5, offenses_6, offenses_7, offenses_8, offenses_9)) %>%
